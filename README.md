...
| `DEX_STREAM_IDLE_TIMEOUT_SECS` | SSE idle watchdog: no chunk (or keep-alive) for this long counts as a stall (default 90, 300 for reasoning-capable models; `0` disables). Stalls and dropped connections retry automatically same-protocol (2 retries) before failing the turn. |
