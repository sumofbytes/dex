...
| `delegate`*        | Spawn a background sub-agent (`explorer`/`reviewer`/`tester`) and return its id immediately; pass `resume_from` to continue a resumable child from its transcript. Omit `model` to inherit this turn's model, or pass `provider/model` when the task's complexity needs a different trade-off. Daemon sessions only.                                                                                 |
