...
pub const CALLBACK_TIMEOUT_SECS: u64 = 180;
...
pub fn token_path(server: &str) -> PathBuf {
...
pub fn load_token(server: &str) -> Option<OAuthToken> {
...
pub fn valid_token(server: &str) -> Option<OAuthToken> {
    load_token(server).filter(|t| t.usable())
}

pub fn save_token(server: &str, tok: &OAuthToken) -> Result<(), String> {
...
pub fn clear_token(server: &str) -> bool {
...
pub fn status_line(server: &str) -> String {
