...
pub fn is_bearer_challenge(header: &str) -> bool {
    let lower = header.to_ascii_lowercase();
...
pub fn ensure_https_or_loopback(url: &str, context: &str) -> Result<(), String> {
    let lower = url.to_ascii_lowercase();
...
pub fn refresh_backoff_active(server: &str) -> bool {
...
pub fn note_refresh_failure(server: &str) {
    refresh_fail_table()
        .lock()
        .unwrap_or_else(|e| e.into_inner())
...
pub fn clear_refresh_backoff(server: &str) {
    refresh_fail_table()
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .remove(server);
...
pub fn split_origin(url: &str) -> (String, String) {
...
pub fn well_known_resource_url(base: &str) -> String {
...
pub async fn refresh_access_token(saved: &OAuthToken) -> Result<OAuthToken, String> {
...
pub fn parse_callback(req: &str) -> Result<(String, String), String> {
...
/// terminals.
pub async fn login(server: &str) -> Result<String, String> {
