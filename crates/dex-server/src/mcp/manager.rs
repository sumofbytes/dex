...
impl McpManager {
    ...
    #[cfg(test)]
    pub async fn insert_client(&self, server: &str, client: McpClient) {
        self.clients
            .write()
            .await
    ...
    pub async fn reconnect(&self, server: &str) -> Result<usize, String> {
        let cfg = self
            .configs
            .get(server)
            .cloned()
    ...
    #[cfg(test)]
    pub async fn tool_definitions(&self) -> Arc<[ToolDefinition]> {
