...
pub fn global_manager() -> Arc<McpManager> {
    GLOBAL
        .get_or_init(|| {
...
pub fn status_line(statuses: &[ServerStatus]) -> Option<String> {
...
pub fn ephemeral_line() -> Option<String> {
    let mgr = GLOBAL.get()?;
...
pub fn cached_statuses() -> Option<Vec<ServerStatus>> {
    let mgr = GLOBAL.get()?;
    try_snapshot(mgr)
}
