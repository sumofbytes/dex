...
pub fn load_provider_entries(file: &Option<serde_yaml::Value>) -> BTreeMap<String, ProviderEntry> {
    let Some(map) = file
        .as_ref()
...
/// `parse_known`).
pub fn known_providers(entries: &BTreeMap<String, ProviderEntry>) -> BTreeSet<String> {
...
pub struct ResolvedProvider {
    ...
    pub api_pin: Option<ApiProtocol>,
...
pub fn setup_guide_error() -> String {
...
pub fn unrouted_selection_error(selection: &str, known: &BTreeSet<String>) -> String {
    let path = config_file_path()
        .map(|p| p.display().to_string())
        .unwrap_or_else(|| "~/.config/dex/config.yaml".to_string());
...
pub fn set_cli_model_overrides(
    model: Option<String>,
...
pub fn cli_overrides() -> (Option<String>, Option<String>, Vec<String>) {
    CLI_OVERRIDES
        .get()
...
pub fn model_api_from_env(selection: &str, bare_id: &str) -> Option<ApiProtocol> {
