...
pub fn row(out: &mut String, key: &str, value: &str, source: &str) {
...
struct ProviderDoctor<'a> {
    pub file: &'a Option<serde_yaml::Value>,
    pub provider_entries: &'a BTreeMap<String, ProviderEntry>,
    ...
    pub pre_model: &'a str,
    pub cfg_result: &'a Result<LlmConfig, Box<dyn std::error::Error>>,
    pub flag_base_url: Option<&'a str>,
    pub permission_override: Option<PermissionMode>,
    pub header_overrides: &'a [String],
