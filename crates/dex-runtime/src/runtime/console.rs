...
pub type LiveApprovalCheck = Arc<dyn Fn(&str) -> bool + Send + Sync>;
...
pub const TOOL_INPUT_COLOR: &str = "\x1b[1;33m";

pub const TOOL_OUTPUT_COLOR: &str = "\x1b[0;34m";

pub const AGENT_COLOR: &str = "\x1b[1;32m";

pub const SPINNER_FRAMES: &[char] = &['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];

pub static CONSOLE_LOCK: Mutex<()> = Mutex::new(());

pub static TOOL_MUTATION_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

pub static SPINNER_RUNNING: AtomicBool = AtomicBool::new(false);

pub static SPINNER_DRAWN: AtomicBool = AtomicBool::new(false);
...
impl Console {
    #[allow(dead_code)]
    pub fn new(sink: mpsc::Sender<SinkLine>, approval: mpsc::Sender<ApprovalRequest>) -> Self {
        Self {
            sink: Some(sink),
    ...
    pub fn daemon(sink: mpsc::Sender<SinkLine>, approval: mpsc::Sender<ApprovalRequest>) -> Self {
        Self {
            sink: Some(sink),
            approval: Some(approval),
            session_approvals: Arc::new(Mutex::new(None)),
    ...
    pub fn with_live_approvals(mut self, live: Option<LiveApprovalCheck>) -> Self {
    ...
    pub fn with_agent(mut self, agent_id: String, name: String) -> Self {
    ...
    pub fn seed_session_approvals(&self, set: std::collections::HashSet<String>) {
        let mut guard = self
            .session_approvals
            .lock()
    ...
    pub fn sink(&self) -> Option<&mpsc::Sender<SinkLine>> {
        self.sink.as_ref()
    }
    ...
    pub async fn emit_async(&self, line: SinkLine) {
        if let Some(sink) = &self.sink {
            let _ = sink.send(line).await;
        }
    }

    pub fn approval_key(name: &str, input: &str) -> String {
    ...
    pub fn session_approved(&self, name: &str, input: &str) -> bool {
    ...
    pub fn record_session_approval(&self, name: &str, input: &str) {
        let key = Self::approval_key(name, input);
        self.session_approvals
            .lock()
            .unwrap_or_else(|e| e.into_inner())
...
pub fn with_console(ui_mode: bool, f: impl FnOnce()) {
    if ui_mode {
...
impl SpinnerGuard {
    pub fn start(console: &Console, label: &str) -> Self {
