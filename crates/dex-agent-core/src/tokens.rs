use dex_ai::{ChatMessage, ToolDefinition};

/// Token overhead per message (role, formatting, turn boundary).
pub const PER_MESSAGE_OVERHEAD: u64 = 12;
...
pub fn estimate_ephemeral_tokens(parts: &[Option<String>]) -> u64 {
...
pub fn message_char_len(message: &ChatMessage) -> usize {
...
impl TokenLedger {
    pub fn rebuild(messages: &[ChatMessage]) -> Self {
...
pub fn schema_token_estimate(defs: &[ToolDefinition]) -> u64 {
...
pub fn schema_chars(defs: &[ToolDefinition]) -> usize {
    defs.iter()
        .map(|d| {
            d.function.name.len()
