...
// Plan types live in `tools::plan` (single source); re-exported here so
// `agent` call sites keep working. Direction is `agent→tools`.
pub(crate) use crate::tools::plan::{
    analyze_plan_transition, parse_plan_progress, parse_plan_steps, post_compaction_reminder,
    PlanProgress, PlanStatus, PlanStep,
};
...
mod tests {
    use super::*;
    use crate::tools::plan::format_plan_snapshot;
    use serde_json::json;
