...
pub(crate) fn extract_file_ops_from_message(msg: &ChatMessage, ops: &mut FileOps) {
...
pub(crate) fn deterministic_summary(
    old: &[ChatMessage],
...
pub(crate) fn attach_file_section(file_ops: &FileOps) -> String {
    let (read_files, modified_files) = compute_file_lists(file_ops);
