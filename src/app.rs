...
fn start_daemon_background() -> std::io::Result<std::net::SocketAddr> {
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async {
            if let Err(e) = crate::daemon::run_daemon(listener).await {
...
fn run_serve(bind: &str) {
    ...
    crate::daemon::prepare_daemon_token(&addr);
...
fn run_run_tool(name: &str, raw_args: &[String]) {
    let parsed = match crate::cli::parse_tool_args(raw_args) {
        Ok(parsed) => parsed,
        Err(error) => {
...
pub fn run() {
    ...
    let mode = crate::cli::resolve_mode(&args);
    if let Err(error) = crate::cli::check_reattach_mode(&args, &mode) {
        eprintln!("error: {error}");
        std::process::exit(1);
    }
    ...
    match mode {
        ...
        Mode::Connect { url } => {
            ...
            let result = match prompt {
                ...
                #[cfg(feature = "tui")]
                None => {
                    crate::ui::run_ratatui_repl_with_remote(&args, &url, false).map_err(Into::into)
                }
                #[cfg(not(feature = "tui"))]
        ...
        Mode::Default => {
            ...
            #[cfg(feature = "tui")]
            if let Err(e) = crate::ui::run_ratatui_repl_with_remote(&args, &url, true) {
                eprintln!("ui error: {}", e);
                std::process::exit(1);
            }
            #[cfg(not(feature = "tui"))]
