...
pub(crate) use crate::workspace::{cached_parse, unique_tmp_path, xdg_path, FileCache};

pub(crate) fn dex_catalog_cache_path() -> Option<std::path::PathBuf> {
...
pub(crate) fn ctx_from_index(model: &str) -> Option<u64> {
...
pub(crate) fn build_ctx_map(catalog: &serde_json::Value) -> BTreeMap<String, u64> {
...
pub(crate) fn write_ctx_index(map: &BTreeMap<String, u64>) {
...
pub(crate) fn ensure_ctx_index(catalog: &serde_json::Value) {
...
pub(crate) fn load_dex_catalog() -> Option<std::sync::Arc<serde_json::Value>> {
...
pub(crate) fn catalog_endpoint_for_model(
    model: &str,
