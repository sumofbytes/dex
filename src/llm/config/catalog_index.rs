...
pub(crate) use crate::workspace::fnv_bytes;
...
pub(crate) struct IndexedModel {
    ...
    pub(crate) cost: Option<CostRates>,
    pub(crate) reasoning_options: Option<Vec<String>>,
...
pub(crate) struct CatalogIndex {
    ...
    pub(crate) by_id: HashMap<String, Vec<IndexedModel>>,
    pub(crate) provider_api: HashMap<String, String>,
    pub(crate) provider_env: HashMap<String, Vec<String>>,
    ...
    pub(crate) bare: Vec<(String, String)>,
    ...
    pub(crate) expanded_for: HashMap<BTreeSet<String>, Vec<String>>,
}
...
pub(crate) fn if_catalog_index_warm<T>(f: impl FnOnce(&CatalogIndex) -> T) -> Option<T> {
...
pub(crate) fn with_catalog_index<T>(f: impl FnOnce(&CatalogIndex) -> T) -> Option<T> {
    let path = dex_catalog_cache_path()?;
    let meta = std::fs::metadata(&path).ok()?;
    let (mtime, len) = (meta.modified().ok()?, meta.len());
    {
...
pub(crate) fn with_catalog_index_mut<T>(f: impl FnOnce(&mut CatalogIndex) -> T) -> Option<T> {
    let path = dex_catalog_cache_path()?;
    let meta = std::fs::metadata(&path).ok()?;
    let (mtime, len) = (meta.modified().ok()?, meta.len());
    {
