...
/// bounds output, and the models.dev catalog prices/target nothing by it.
/// A model with a lower output cap fails with a named 400, not a hang.
...
pub(crate) fn messages_body(
    ...
    let (system, msgs) = messages_input(messages);
    let mut body = json!({
        "model": config.model,
    ...
    if let Some(effort) = &config.thinking_effort {
        ...
        // The budget must sit strictly below max_tokens.
        body["max_tokens"] = json!(DEFAULT_MAX_TOKENS.max(budget + 4096));
...
mod tests {
    ...
    fn messages_body_shape_and_thinking_budgets() {
        ...
        assert_eq!(body["max_tokens"], 16_384);
        assert_eq!(body["tools"].as_array().unwrap()[0]["name"], "read");
        ...
        // System content lands in the top-level `system` string.
